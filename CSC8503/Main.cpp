...
class IntroScreen : public PushdownState {
	...
	void OnAwake() override {
		std::cout << "Welcome to a realy awsome game" << std::endl;
...
void TestStateMachine() {
	...
	// define how machine will switch between the states
...
void TestPathfinding() {
	...
	// x is right and left
	// z is up and down (reversed)
	// read coordinates with (0,0) being top right corner
	Vector3 startPos(180, 0, 10);
	Vector3 endPos(10, 0, 190);
...
void TestBehaviorTree() {
	...
	BehaviourAction* lookForTreasure = new BehaviourAction("Look For Treasure", [&](float dt, BehaviourState state)->BehaviourState {
		if (state == Initialise) {
			...
			std::cout << "No treasure found here :-(" << std::endl;
			return Failure;
		}
		return state;
		});
	...
	// run five times from start
	for (int i = 0; i < 5; i++) {
		rootSequence->Reset();
		behaviourTimer = 0.0f;
		distanceToTarget = rand() % 250;
		BehaviourState state = Ongoing;
		std::cout << "Were going on an adventure" << std::endl;
		while (state == Ongoing)
...
void TestNetworking() {
	...
	for (int i = 0; i < 100; i++) {
		StringPacket stringPacketServer = StringPacket("Server Says Hello! " + std::to_string(i));
		server->SendGlobalPacket(stringPacketServer);

		StringPacket stringPacketClient = StringPacket("Client Says Hello! " + std::to_string(i));
		client->SendPacket(stringPacketClient);
...
int main() {
	...
	PushdownMachine pushdownMachine(new MainMenu(g));
